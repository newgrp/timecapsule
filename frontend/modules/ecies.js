...
async function eciesDerive(pub, priv) {
    const hkdfKey = await crypto.subtle.deriveKey(
        ...
        /*baseKey=*/priv,
        /*derivedKeyType=*/{ name: "HKDF", hash: "SHA-256", salt: new ArrayBuffer(), info: new ArrayBuffer() },
        /*extractable=*/false,
        /*keyUsages=*/["deriveBits"]
    )
    const hkdfRaw = await crypto.subtle.deriveBits(
        /*algorithm=*/{ name: "HKDF", hash: "SHA-256", salt: new ArrayBuffer(), info: new ArrayBuffer() },
        /*baseKey=*/hkdfKey,
...
export async function eciesEncrypt(pubKey, plaintext) {
    ...
    if (ephemeralPub.byteLength != ecSEC1PubKeySize) {
        throw new Error("Ephemeral public key encoded to wrong length: " + ephemeralPub.byteLength)
    }
    ...
    const ciphertext = await crypto.subtle.encrypt(
        /*algorithm=*/{ name: "AES-CTR", counter: ctrBlock, length: aesCounterBits },
    ...
    const hmac = await crypto.subtle.sign(
        ...
        /*data=*/(await concatByteArrays(ctrBlock, ciphertext)).buffer
    )
...
export async function eciesDecrypt(privKey, cipherBlob) {
    if (cipherBlob.byteLength < ecSEC1PubKeySize + aesCtrBlockSize + hmacSHA256Size) {
        throw new Error("Ciphertext is too short")
    }

    const ephemeralPub = cipherBlob.slice(0, ecSEC1PubKeySize)
    const ctrBlock = cipherBlob.slice(ecSEC1PubKeySize, ecSEC1PubKeySize + aesCtrBlockSize)
    const ciphertext = cipherBlob.slice(ecSEC1PubKeySize + aesCtrBlockSize, -hmacSHA256Size)
